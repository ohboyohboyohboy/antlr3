...
* the script scripts/gem-bundle.rb runs the gem bundle command to update
  the project's local gem installations. Running `rake dev:setup' will
...
|-- benchmark  : a handful of simple benchmarking scripts
`-- profile    : several full ruby-prof code profiling scripts
...
* tests are further organized by specific subject, with one or more tests
  with a specific focused grouped together in a single file under the category
