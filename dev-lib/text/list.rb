...
class List
  ...
  def spacer( value = nil )
    ...
    @spacer or DEFAULT_COLUMN_SPACER
  end
...
private
  ...
  def render_content( output )
    ...
    columns = each_column.zip( widths ).map do | width, cells |
      if cells.length < number_of_rows
        cells.fill( SingleWord.new( '' ), cells.length,
  ...
  def optimize_columns( limit )
    ...
    for number_of_rows in ( 1 .. c )
      ...
      total_width = columns.inject( ( number_of_columns - 1 ) * spacing, :+ )
