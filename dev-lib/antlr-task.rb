...
class CompileTask < Rake::TaskLib
  ...
  def initialize( *grammar_files )
    ...
    @grammars = GrammarSet.new( grammar_files, options )
    @name = options.fetch( :name, 'antlr' )
    if block_given?
      yield( @grammars )
      define
    end
  ...
  def define
    namespace( @name ) do
      ...
      @grammars.define_tasks
      
      desc( "compile ANTLR grammars" )
      task( 'compile' => @grammars.target_files )
    end
  end
end
...
class GrammarSet
  ...
  def clean
    ...
    if ( Dir.entries( output_directory ) - %w( . .. ) ).empty?
