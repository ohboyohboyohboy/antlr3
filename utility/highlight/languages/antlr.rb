...
module Languages
class ANTLR
  STYLE_SHEET = File.read Highlight.style_path('ANTLR3.css')
  CATEGORIES = {}
  ...
  add_category('o', REWRITE, ROOT, BANG, RANGE,
               ...
               "'=>'", "'~'", "'.'", "'?'", "'+'", "'*'")
  add_category('k', SCOPE, FRAGMENT, KEYWORD, "'catch'", "'finally'",
               "'grammar'", "'lexer'", "'parser'", "'private'", "'protected'", "'public'", "'throws'", "'tree'", OPTIONS, TOKENS)
  ...
  def initialize(source, options = {})
    ...
    if rule_name = options[:snippet]
      @tree = parser.send(rule_name).tree
      @name = options[:name] || "#{rule_name}-#{__id__.abs}"
    else
      @tree = parser.grammar_def.tree
      @name = options[:name] || @tree[0].text
    end
  ...
  def full_document(out = StringIO.new)
    ...
    out.puts   STYLE_SHEET.fixed_indent(6)
    out.puts   '    </style>'
    out.puts   '  </head>'
    out.puts   '  <body>'
