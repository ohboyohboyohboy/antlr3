...
class ColorCodedProgressBar < ProgressBar
  attr_reader :current, :total
  def initialize(count)
    super('', count)
    @bar_length = screen_width(@out) - 50
    @format_arguments = [:title, :current, :total, :percentage, :bar, :stat]
    @format = "%-25s %3i/%3i %3d%% %s %s".send(:cyan)
  end
  ...
  STATUS_COLORS.each do |status, color|
    define_method(status) do
      @format = "%-25s %3i/%3i %3d%% %s %s".send(color)
      inc
    end
  end
  ...
  def wipe
    @out.print(' ' * (screen_width(@out) - 1) << "\r")
    @out.flush
  end
end